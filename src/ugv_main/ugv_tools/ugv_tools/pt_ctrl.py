...
class PtCtrl(Node):
    def __init__(self, name):
        ...
        self.input_thread = threading.Thread(target=self.handle_inputs, daemon=True)
    ...
    def handle_inputs(self):
        """Thread function to handle user inputs without blocking callbacks"""
        while rclpy.ok():
            try:
                # Wait for a signal to get new coordinates
                action = self.input_queue.get()
                if action == "get_coords":
                    try:
                        coords = input("Enter the 4 x and y coordinates (separated by space): ").split()
                        # Put the coordinates into the queue for the main thread to use
                        self.coord_queue.put(coords)
                    except Exception as e:
                        self.get_logger().error(f"Error getting coordinates: {e}")
                        self.coord_queue.put(None)  # Signal that we didn't get valid coordinates
            except Exception as e:
